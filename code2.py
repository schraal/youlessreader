...
def ObtainDate(fac):
    while True:
        try:
            t=datetime.datetime.strptime(myform['Day'].value+' '+myform['Month'].value+' '+myform['Year'].value, "%d %B %Y")
...
def handlepage(t):
    ...
    try:
        con,cur=openMySQLdb()
        query=cur.execute('SELECT datetime,watt FROM data WHERE DATE(datetime) BETWEEN "'+datestringBegin+'" and "'+datestringEnd+'"')
